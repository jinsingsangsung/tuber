...
from models.sparse_jhmdb import build_model
...
def main_worker(cfg):
    ...
    # create model
    print('Creating SparseRCNN model')
    model, criterion, postprocessors = build_model(cfg)
    model = deploy_model(model, cfg, is_tuber=False)
    ...
    for epoch in range(cfg.CONFIG.TRAIN.START_EPOCH, cfg.CONFIG.TRAIN.EPOCH_NUM):
        ...
        if cfg.DDP_CONFIG.GPU_WORLD_RANK == 0 and (cfg.CONFIG.LOG.SAVE_FREQ == 0 or epoch cfg.CONFIG.TRAIN.EPOCH_NUM - 1):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config-file',
                        default='./configuration/Sparse_CSN50_JHMDB.yaml',
