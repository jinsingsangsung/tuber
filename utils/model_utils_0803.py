...
def load_detr_weights(model, pretrain_dir, cfg):
    ...
    checkpoint = torch.load("../pretrained_models/transformer/og-dab-detr.pth")
    ...
    for k, v in checkpoint['model'].items():
        if 'transformer.encoder' in k:
            if "self_attn.out_proj" in k:
                pretrained_dict.update({k.replace("self_attn", "self_attn_s"): v})
            elif "activation" in k:
                pass
            elif "query_scale" in k:
                pretrained_dict.update({k: v})    
            else:
                weight_name = k.split(".")[-2]
                pretrained_dict.update({k.replace(weight_name, weight_name+"_s"): v})
        elif 'transformer.decoder.layers' in k:
            if "activation" in k:
                pass
            else:
                pretrained_dict.update({k: v})
        elif 'transformer.decoder.norm' in k:
            pretrained_dict.update({k.replace("norm", "cls_norm"): v})
        elif len(k.split(".")) >= 4 and k.split('.')[-4] == 'bbox_embed':
            pretrained_dict.update({k: v})
        # elif k.split('.')[-2] == 'refpoint_embed':
        ...
        #     # pretrained_dict.update({k: v[:query_size]})
            ...
            if not cfg.CONFIG.MODEL.SINGLE_FRAME:
                ...
                query_size = cfg.CONFIG.MODEL.QUERY_NUM
                pretrained_dict.update({k:v[:query_size].repeat(cfg.CONFIG.MODEL.TEMP_LEN, 1)})
            # print(v.shape) # v의 len이 135임! 따라서 query size > 135면 error남
            if cfg.DDP_CONFIG.GPU_WORLD_RANK == 0:
                print_log(log_path, "query_size:", query_size)
            # pretrained_dict.update({k: v[:query_size]})
    
    # for k, v in checkpoint2['model'].items():
    #     if ("transformer.decoder.layers" in k) and (not "self_attn" in k):
    #         if ("cross_attn.attention_weights" in k) or ("sampling_offsets" in k):
    #             pretrained_dict.update({"module." + k.replace("layers", "cls_layers"): v.chunk(4, dim=0)[-1]})
    #         else:
    #             pretrained_dict.update({"module." + k.replace("layers", "cls_layers"): v})
        # if "ref_point_head" in k:
        #     pretrained_dict.update({"module." + k.replace("point", "anchor"): v})
    if distributed:
        # print("current model: ",[k for k in model_dict.keys()][:10])
        # print("from the pretrained model", [k for k in pretrained_dict.keys()][:10])
    ...
    if cfg.DDP_CONFIG.GPU_WORLD_RANK == 0:
        print_log(log_path, "number of detr unused model layers:", unused_dict.keys())
