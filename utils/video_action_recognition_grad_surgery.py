...
def train_tuber_detection(cfg, model, criterion, data_loader, optimizer, epoch, max_norm, lr_scheduler, writer=None):
    ...
    for idx, data in enumerate(data_loader):
        ...
        losses_cls = sum(loss_dict[k] * weight_dict[k] for k in loss_dict.keys() if (k in weight_dict) and ("ce" in k) and (not "ce_b" in k))
        losses_loc = sum(loss_dict[k] * weight_dict[k] for k in loss_dict.keys() if (k in weight_dict) and (("giou" in k) or ("ce_b" in k) or ("bbox" in k)))
        losses = [losses_cls, losses_loc]
        optimizer.zero_grad()
        for loss in losses:
            optimizer._optim.zero_grad(set_to_none=True)
        ...
        if cfg.DDP_CONFIG.GPU_WORLD_RANK == 0:
            if idx % cfg.CONFIG.LOG.DISPLAY_FREQ == 0:
                ...
                for param in optimizer._optim.param_groups:
                    lr = param['lr']
