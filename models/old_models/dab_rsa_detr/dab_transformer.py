...
class Transformer(nn.Module):
    ...
    def __init__(self, d_model=512, nhead=8, num_queries=300, num_encoder_layers=6,
        ...
        encoder_layer = RSAEncoderLayer(d_model)
        self.encoder = RSAEncoder(encoder_layer, num_encoder_layers, d_model, encoder_norm)
    ...
    def forward(self, src, mask, refpoint_embed, pos_embed):
        ...
        memory = memory.permute(0,2,1,3,4).contiguous()
        memory = memory.reshape(bs, t, c, h, w).flatten(0,1)
...
class RSAEncoderLayer(nn.Module):
    def __init__(self, d_model=256, activation="relu"):
        super().__init__()
        self.rsa = RSA(
                256, 256,
                nh=8,
...
class RSAEncoder(nn.Module):
    ...
    def __init__(self, encoder_layer, num_layers=1, d_model=256, norm=None):
        super().__init__()
