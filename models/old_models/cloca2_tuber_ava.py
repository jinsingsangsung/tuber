...
from models.cloca2_transformer.transformer import build_transformer, build_transformer2, TransformerDecoder, TransformerDecoderLayer
from models.cloca2_transformer.criterion import SetCriterion, PostProcess, SetCriterionAVA, PostProcessAVA, MLP


class DETR(nn.Module):
    """ This is the DETR module that performs object detection """
    def __init__(self, backbone, transformer, transformer2, num_classes, num_queries,
        ...
        if self.dataset_mode != 'ava':
            ...
            self.class_query_embed = nn.Embedding(num_classes * temporal_length, hidden_dim)
        else:
    ...
    def forward(self, samples: NestedTensor):
        ...
        hs, enc_feature, _, attn_decoder = self.class_transformer(self.input_proj(src), mask, self.class_query_embed.weight, pos[-1])
        hs2, dec_attn = self.transformer(enc_feature, mask, self.loc_query_embed.weight, pos[-1])

        outputs_class = torch.einsum('bcd,bdn->bnc', attn_decoder, dec_attn.permute(0,2,1))
        # outputs_class = self.class_fc(self.dropout(hs))
        ...
        # q_class = self.cross_attn(self.class_query_embed.weight, enc_feature, enc_feature, attn_decoder)[0]
        ...
        # pred_logits_b: bs, num_classes. 3
        if self.aux_loss:
...
def build_model(cfg):
    ...
    transformer2 = build_transformer2(cfg)
