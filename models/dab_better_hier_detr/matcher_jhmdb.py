...
def batched_index(input_tensor, index):
    ...
    index: bs (each element is nonnegative integer < num_classes)
    """
    B = input_tensor.shape[0]
    assert index.shape[0] == B
    output_tensor = torch.zeros(input_tensor.shape[:2] + (1,), device=input_tensor.device)
    for b in range(B):
        output_tensor[b] = input_tensor[b][:, index[b]][:, None]
...
class HungarianMatcher(nn.Module):
    ...
    def forward(self, outputs, targets):
        ...
        out_prob = outputs["pred_logits"].flatten(0, 1).sigmoid() # bs*t, nq, 22
        ...
        # bs*t, nq, num_classes
        cost_class = cost_class.reshape(bs, t, num_queries, -1)
