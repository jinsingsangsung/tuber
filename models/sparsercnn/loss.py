...
from utils.misc import (NestedTensor, nested_tensor_from_tensor_list,
                       ...
                       is_dist_avail_and_initialized, accuracy_sigmoid)
...
class SetCriterion(nn.Module):
    ...
    def __init__(self, cfg, num_classes, matcher, weight_dict, eos_coef, losses, use_focal):
        ...
        if self.use_focal:
            self.focal_loss_alpha = cfg.CONFIG.MODEL.SparseRCNN.ALPHA
            self.focal_loss_gamma = cfg.CONFIG.MODEL.SparseRCNN.GAMMA
        else:
...
class SetCriterionAVA(nn.Module):
    ...
    def loss_boxes(self, outputs, targets, indices, num_boxes):
        ...
        image_size = torch.cat([t["orig_size"].unsqueeze(0).repeat(len(t["boxes"]),2) for t, (_,i) in zip(targets, indices)], dim=0)

        src_boxes_ = src_boxes / image_size
        loss_bbox = F.l1_loss(src_boxes_, target_boxes, reduction='none')
