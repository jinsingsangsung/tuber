...
class TransformerDecoder(nn.Module):
    ...
    def __init__(self, decoder_layer, num_layers, norm=None, return_intermediate=False, 
        ...
        self.cls_norm2 = nn.LayerNorm(2*d_model)
    ...
    def forward(self, tgt, memory,
        ...
        for layer_id, layer in enumerate(self.layers):
            ...
            cls_feature = self.conv_activation(cls_feature)

            cls_feature = self.conv2(cls_feature)
            # cls_feature = self.bn2(cls_feature)
            cls_feature = self.conv_activation(cls_feature)

            cls_feature = self.conv3(cls_feature)
            # cls_feature = self.bn3(cls_feature)
            cls_feature = self.conv_activation(cls_feature)
            cls_output = cls_feature.mean(dim=(2,3)).view(len(tgt), -1, cls_feature.shape[1])
