...
class TransformerDecoder(nn.Module):
    ...
    def forward(self, query, key, value,
        ...
        output = query
        for layer in self.layers:
            key, value, output, attn = layer(query=output, key=key, value=value, src_mask=mask,
                                 src_key_padding_mask=src_key_padding_mask, pos=pos)
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward_post(self,
                     ...
                     pos: Optional[Tensor] = None,
                     mask_p = 0.7):
        nb, lay_n, d = query.shape # lay_n is in fact lay_n * b
        if key.shape[0] != nb: #first layer
            thw = key.shape[0] # thw = t*h*w
            q_output, dec_attn = self.cross_attn(query, key, value)
            # dec_attn: lay_n*bs, nb, thw
        else:
            thw = key.shape[1]
            key = key.flatten(0,1) # nb*thw, lay_n*bs, d
            value = value.flatten(0,1)
            attn_mask = torch.ones(nb, key.size(0), device=query.device) > 0
            for n in range(nb):
                attn_mask[n, n*thw:n*thw+thw] = False
            q_output, dec_attn = self.cross_attn(query, key, value, attn_mask=attn_mask)
            dec_attn_recon = torch.empty((query.size(1),nb, thw), device=query.device)
            for n in range(nb):
                dec_attn_recon[:,n,:] = dec_attn[:, n, n*thw:n*thw+thw]
            dec_attn = dec_attn_recon # dec_attn: lay_n*bs, nb, q_thw

        filtered_indices = dec_attn.topk(int(mask_p*thw), -1, largest=False, sorted=False)[1] # 6, 15, 1299
        filtered_indices = filtered_indices.flatten(0,1)
        key = key.unsqueeze(0).repeat(nb, 1, 1, 1).permute(2, 0, 1, 3).contiguous().flatten(0,1) # 6 * 15, 1856, 256
    ...
    def forward(self, query, key, value,
                ...
                pos: Optional[Tensor] = None):
        if self.normalize_before:
            return self.forward_pre(query, key, value, src_mask, src_key_padding_mask, pos)
        return self.forward_post(query, key, value, src_mask, src_key_padding_mask, pos)