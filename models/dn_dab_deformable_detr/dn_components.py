...
def prepare_for_dn(dn_args, tgt_weight, embedweight, batch_size, training, num_queries, num_classes, hidden_dim, label_enc):
    ...
    :param label_enc: encode labels in dn (nn.Linear(81, 255))
    :return:
    """
    ...
    if training:
        ...
        num_instances = [known[i].size(0) for i in range(bs)]
        # length: bsz, each element is torch.tensor of size num_instance
        # ex) targets[0]['labels']: torch.tensor([75, 0]) if num_instance=2
        know_idx = [torch.nonzero(t) for t in known]
        # known_num = [sum(k) for k in known]
        known_num = num_instances
        ...
        boxes = torch.cat([t['boxes'][:, 1:] for t in targets])
        ...
        known_labels = labels.repeat(scalar, 1)
        ...
        if label_noise_scale > 0:
            ...
            chosen_indice = torch.nonzero(p < (label_noise_scale))  # usually half of bbox noise
            # chosen_indice = torch.nonzero(p < (label_noise_scale)).view(-1)  # usually half of bbox noise
            # new_label = torch.randint_like(chosen_indice, 0, num_classes)  # randomly put a new one here
            # known_labels_expaned.scatter_(0, chosen_indice, new_label)
            # flipping trick
            known_labels_expaned[chosen_indice[:, 0], chosen_indice[:, 1]] *= -1
            known_labels_expaned[chosen_indice[:, 0], chosen_indice[:, 1]] += 1 
        ...
        embeddings = [torch.zeros(1, hidden_dim-1).float().to('cuda') for _ in range(len(m))]
        for i, indices in enumerate(m):
            axes = torch.nonzero(indices)
            for ax in axes:
                embeddings[i] += label_enc(ax) / len(axes)
        input_label_embed = torch.cat(embeddings, dim=0)

        # add dn part indicator
        indicator1 = torch.ones([input_label_embed.shape[0], 1]).cuda()
        input_label_embed = torch.cat([input_label_embed, indicator1], dim=1) # denoised group indicator
...
def dn_post_process(outputs_class, outputs_coord, outputs_class_b, mask_dict):
    """
    ...
    if mask_dict and mask_dict['pad_size'] > 0:
        ...
        outputs_class_b = outputs_class_b[:, :, mask_dict['pad_size']:, :]
        outputs_coord = outputs_coord[:, :, mask_dict['pad_size']:, :]
        mask_dict['output_known_lbs_bboxes']=(output_known_class,output_known_coord)
    return outputs_class, outputs_coord, outputs_class_b
