...
from models.conv_transformer.transformer_layers import TransformerEncoderLayer, TransformerEncoder
...
class ConvBlock(nn.Module):
    def __init__(self, dim, drop_path=0):
        super().__init__()
        self.conv1 = nn.Conv3d(dim, dim, kernel_size=(3,3,3), padding=1)
...
class DETR(nn.Module):
    ...
    def __init__(self, backbone, transformer, num_classes, num_queries, num_frames,
        ...
        self.conv1 = nn.Conv3d(2*hidden_dim, hidden_dim, kernel_size=1)
        conv_block = ConvBlock(hidden_dim, 0)
        self.conv_blocks = nn.ModuleList([conv_block for _ in range(num_conv_blocks)])
        assert cls_mode in ["conv-trans", "cls-queries"]
        self.cls_mode = cls_mode
        if cls_mode == "cls-queries":
            self.class_queries = self.class_embed.weight
        self.q_proj = nn.Conv3d(hidden_dim, hidden_dim, kernel_size=1)
        self.k_proj = nn.Conv3d(hidden_dim, hidden_dim, kernel_size=1)
        self.v_proj = nn.Conv3d(hidden_dim, hidden_dim, kernel_size=1)
        self.linear = nn.Linear(hidden_dim, hidden_dim)
        self.cls_norm_ = nn.LayerNorm(hidden_dim)
        self.cls_linear1_ = nn.Linear(hidden_dim, 4*hidden_dim)
        self.cls_linear2_ = nn.Linear(4*hidden_dim, hidden_dim)
    ...
    def forward(self, samples: NestedTensor, targets=None):
        ...
        actor_feature_expanded = hs_query.flatten(0,1)[..., None, None, None].expand(-1, -1, *src_c.shape[-3:]) # nq*lay_n*bs, D, T, H, W
        encoded_feature_expanded = src_flatten[:, None].expand(-1, nq, -1, -1).flatten(1,2).permute(1,2,0).contiguous().reshape(-1, dim, *src_c.shape[-3:])
        cls_feature = self.conv1(torch.cat([actor_feature_expanded, encoded_feature_expanded], dim=1))
        for block in self.conv_blocks:
            cls_feature = block(cls_feature)

        
        if self.cls_mode == "conv-trans":
            query = self.q_proj(cls_feature)
            key = self.k_proj(encoded_feature_expanded)
            value = self.v_proj(encoded_feature_expanded)
            attn = (query*key).sum(dim=1).flatten(1).softmax(dim=1).reshape(-1,1,*src_c.shape[-3:])
            cls_output = (attn * value).sum(dim=-1).sum(dim=-1).sum(dim=-1) #
            ...
            cls_output = self.cls_norm_(cls_output).view(nq, -1, dim)
            cls_output = cls_output.permute(1, 0, 2).contiguous().view(lay_n, bs, nq, self.hidden_dim)
            outputs_class = self.class_embed(self.dropout(cls_output))
            
        else: # self.cls_mode == "cls-queries"
            # query = self.class_queries[None, :, :, None, None, None].expand(nq*lay_n*bs, -1, -1, *src_c.shape[-3:])
            # key = self.k_proj(cls_feature)[:, None].expand(-1, self.num_classes, -1, -1, -1, -1)
            # value = self.v_proj(encoded_feature_expanded)[:, None]
            # attn = torch.einsum("bncthw,bncthw->bcthw", query, key).flatten(2).softmax(dim=2).reshape(nq*lay_n*bs, -1, *src_c.shape[-3:])[:, :, None]

            q = self.class_queries[:, None].expand(-1, nq*lay_n*bs, -1) # N_c, nq*lay_n*bs, D
            k = v = cls_feature.flatten(2).permute(2,0,1)
            cls_output = self.cross_attn(q, k, v)[0]
            # attn = (query*key).sum(dim=2).flatten(2).softmax(dim=2).reshape(nq*lay_n*bs, -1, *src_c.shape[-3:])[:, :, None]
            # cls_output = (attn * value).sum(dim=-1).sum(dim=-1).view(nq, -1, self.num_classes, cls_feature.shape[1]) #N_q, B, N_c, D
            cls_output = self.linear(cls_output)
            cls_output2 = self.cls_linear2_(self.dropout_(self.activation(self.cls_linear1_(cls_output))))
            cls_output = cls_output + self.dropout_(cls_output2)
            cls_output = self.cls_norm_(cls_output)
            cls_output = cls_output.permute(1, 0, 2).contiguous().view(lay_n, bs, nq, -1, self.hidden_dim)
            outputs_class = self.dropout(cls_output).mean(-1)
