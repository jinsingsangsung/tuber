...
class Transformer(nn.Module):
    ...
    def __init__(self, d_model=512, nhead=8, num_queries=300, num_encoder_layers=6,
        ...
        decoder_layer2 = TransformerDecoderLayer(d_model, nhead, dim_feedforward,
                                                dropout, activation, normalize_before, keep_query_pos=keep_query_pos, second=True)
        decoder_norm2 = nn.LayerNorm(d_model)
        self.decoder2 = TransformerDecoder(decoder_layer2, 1, decoder_norm2,
                                          return_intermediate=return_intermediate_dec,
                                          d_model=d_model, query_dim=query_dim, keep_query_pos=keep_query_pos, query_scale_type=query_scale_type,
                                          modulate_hw_attn=modulate_hw_attn,
                                          bbox_embed_diff_each_layer=bbox_embed_diff_each_layer)
    ...
    def forward(self, src, mask, refpoint_embed, pos_embed, sub_refpoint_embed):
        # flatten NxCxHxW to HWxNxC
        bs, c, t, h, w = src.shape
        orig_shape = src.shape
        assert t != 1
        # src = src.view(bs, c, t, h, w)
        src = src.flatten(2).permute(2, 0, 1) # thw, b, c
        pos_embed = pos_embed.flatten(2).permute(2, 0, 1)
        refpoint_embed = refpoint_embed.unsqueeze(1).repeat(1, bs, 1)
        sub_refpoint_embed = sub_refpoint_embed.unsqueeze(1).repeat(1, bs, 1)
        mask = mask.flatten(1).permute(1, 0) # thw, b
        memory = self.encoder(src, src_key_padding_mask=mask, pos=pos_embed, orig_shape=orig_shape)
        # memory: thw, b, c
        ...
        hs, references, out, ref = self.decoder(tgt, memory, memory_key_padding_mask=mask,
                          pos=pos_embed, refpoints_unsigmoid=refpoint_embed, orig_shape=orig_shape)
        M = sub_refpoint_embed.size(0) // ref.size(0)
        
        refpoint_embed2 = ref.repeat(M, 1, 1) + sub_refpoint_embed
        out = out.repeat(M, 1, 1)
        hs2, references2, _, ref = self.decoder2(out, memory, memory_key_padding_mask=mask,
                          pos=pos_embed, refpoints_unsigmoid=refpoint_embed2, orig_shape=orig_shape)
        hs2 = hs2.view(-1, bs, M, num_queries, c).mean(2)
    ...
    def forward(self, src,
        ...
        for layer_id, layer in enumerate(self.layers):
            ...
            output = layer(output, src_mask=mask,
                           src_key_padding_mask=src_key_padding_mask, pos=pos*pos_scales, orig_shape=orig_shape)
    ...
    def forward(self, tgt, memory,
        ...
        for layer_id, layer in enumerate(self.layers):
            ...
            if self.bbox_embed is not None:
                ...
                recent_reference_points = inverse_sigmoid(new_reference_points)
...
class TransformerEncoderLayer(nn.Module):
    ...
    def forward(self,
                ...
                pos: Optional[Tensor] = None,
                orig_shape = None):
        bs, ch, t, h, w = orig_shape
        ###### attend across h*w ######
        src_t = src.view(t, h * w, bs, ch).permute(1, 0, 2, 3).contiguous().view(h * w, -1, ch)
        pos_t = pos.view(t, h * w, bs, ch).permute(1, 0, 2, 3).contiguous().view(h * w, -1, ch)
        src_key_padding_mask_t = src_key_padding_mask.view(t, h * w, bs).permute(0, 2, 1).contiguous().view(-1, h*w) # t*bs, hw
        q_t = k_t = self.with_pos_embed(src_t, pos_t)
        src2_t = self.self_attn_t(q_t, k_t, value=src_t, attn_mask=src_mask,
                                  key_padding_mask=src_key_padding_mask_t)[0]
        src_t = src_t + self.dropout1(src2_t)
        src_t = self.norm1_t(src_t).view(h * w, t, bs, ch).permute(1, 0, 2, 3).contiguous().view(t * h * w, bs, ch)
        
        ###### attend across t ######
        src_s = src.reshape(t, h * w * bs, ch)
        pos_s = pos.reshape(t, h * w * bs, ch)
        q_s = k_s = self.with_pos_embed(src_s, pos_s)
        src_key_padding_mask_t = src_key_padding_mask.view(t, h * w, bs).permute(1, 2, 0).contiguous().view(-1, t) # hw*bs, t
...
class TransformerDecoderLayer(nn.Module):
    ...
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1,
                 ...
                 rm_self_attn_decoder=False, second=False):
        super().__init__()
    ...
    def forward(self, tgt, memory,
        ...
        memory_key_padding_mask = memory_key_padding_mask.permute(1,0).contiguous()
        if self.second:
            ...
            # q: NT, B, D
            # k: THW, B, D
            attn_mask = torch.ones((q.size(0), k.size(0)), device=q.device)
            b, c, t, h, w = orig_shape
            hw = h * w
            n = q.size(0) // t
            for t_ in range(t):
                attn_mask[t_::n, t_*hw:(t_+1)*hw] = 0

            tgt2 = self.cross_attn(query=q,
                                    key=k,
                                    value=v, attn_mask=attn_mask>0,
                                    key_padding_mask=memory_key_padding_mask,
                                    )[0]
