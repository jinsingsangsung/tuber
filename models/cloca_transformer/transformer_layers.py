...
class TransformerEncoderLayer(nn.Module):
    ...
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1,
                 activation="relu", normalize_before=False, crop_p = 0.7):
        super().__init__()
    ...
    def forward_post(self,
        ...
        src2_t, enc_attn_t = self.self_attn_t(q_t, k_t, value=src_t, attn_mask=src_mask,
        ...
        src2_s, enc_attn_s = self.self_attn_s(q_s, k_s, value=src_s, attn_mask=src_mask,
                                  key_padding_mask=src_key_padding_mask)
...
class TransformerDecoder(nn.Module):
    ...
    def __init__(self, decoder_layer, num_layers, dropout=0.1, norm=None):
        super().__init__()
        self.layers = _get_clones(decoder_layer, num_layers)
        self.num_layers = num_layers
        self.norm = norm

    def forward(self, query, key, value, attn_weight,
                mask: Optional[Tensor] = None,
                src_key_padding_mask: Optional[Tensor] = None,
                pos: Optional[Tensor] = None):
        output = query
        for layer in self.layers:
            key, value, output, dec_attn = layer(query=output, key=key, value=value, src_mask=mask, attn_weight=attn_weight,
                                 src_key_padding_mask=src_key_padding_mask, pos=pos)

        if self.norm is not None:
            output = self.norm(output)

        return output, dec_attn
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward_post(self,
        ...
        if key.size(1) == d: # first layer of the decoder
            bs, _, t, h, w = key.shape

            key = key.flatten(2,4).permute(2,0,1).contiguous() #thw, bs, d
            value = value.flatten(2,4).permute(2,0,1).contiguous() #thw, bs, d
            thw = key.size(0)

            assert attn_weight.size(-1) == h*w
            ...
            key = key.unsqueeze(0).repeat(nb, 1, 1, 1).permute(2, 0, 1, 3).contiguous().flatten(0,1) # bs*nb thw d
            value = value.unsqueeze(0).repeat(nb, 1, 1, 1).permute(2, 0, 1, 3).contiguous().flatten(0,1)

            new_key = torch.stack([key[..., i].gather(1, filtered_indices) for i in range(key.size(-1))], -1).view(nb, -1, d).flatten(0,1)
            new_value = torch.stack([value[..., i].gather(1, filtered_indices) for i in range(value.size(-1))], -1).view(nb, -1, d).flatten(0,1)

            attn_mask = torch.ones(nb, new_key.size(0), device=query.device) > 0
            for n in range(nb):
                attn_mask[n, n*new_thw:n*new_thw+new_thw] = False

            q_output, dec_attn = self.cross_attn(query, new_key, new_value, attn_mask=attn_mask)
            
            return new_key, new_value, q_output, dec_attn

        else:
            thw = key.size(0)
            attn_mask = torch.ones(nb, thw, device=query.device) > 0
            for n in range(nb):
                attn_mask[n, n*thw:n*thw+thw] = False
            q_output, dec_attn = self.cross_attn(query, key, value)

            return key, value, q_output, dec_attn
    ...
    def forward(self, query, key, value, attn_weight,
                ...
                pos: Optional[Tensor] = None):
        if self.normalize_before:
            return self.forward_pre(query, key, value, src_mask, attn_weight, src_key_padding_mask, pos)
        return self.forward_post(query, key, value, attn_weight, src_mask, src_key_padding_mask, pos)