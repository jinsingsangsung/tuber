...
class DeformableTransformerDecoder(nn.Module):
    def __init__(self, decoder_layer, num_layers, return_intermediate=False, use_dab=False, d_model=256, high_dim_query_update=False, no_sine_embed=False):
        ...
        if use_dab:
            ...
            if self.no_sine_embed:
                self.ref_point_head = MLP(4, d_model, d_model, 4)
            else:
    ...
    def forward(self, tgt, reference_points, src, src_spatio_temporal_shapes,       
        ...
        if reference_points.size(0) != bs:
            reference_points = reference_points[None].repeat(bs, 1, 1) # bs, nq x T, 4(xywh)
        ...
        for lid, layer in enumerate(self.layers):
            ...
            if reference_points.shape[-1] == 4:
                reference_points_input = reference_points[:, :, None] * torch.cat([src_valid_ratios[..., 1:], src_valid_ratios[..., 1:]], -1)[:, None] # bs, nqxT, 4, 4
            else:
            ...
            if self.use_dab:
                ...
                if self.no_sine_embed:
                    ...
                    query_sine_embed = gen_sineembed_for_position(reference_points_input[:, :, 0, :]) # bs, nqxT, 256*2 
                    raw_query_pos = self.ref_point_head(query_sine_embed) # bs, nqxT, 256
...
def gen_sineembed_for_position3d(pos_tensor):
    # n_query, bs, _ = pos_tensor.size()
    # sineembed_tensor = torch.zeros(n_query, bs, 256)
    scale = 2 * math.pi
    dim_t = torch.arange(128, dtype=torch.float32, device=pos_tensor.device)
