...
class Transformer(nn.Module):
    ...
    def forward(self, src, mask, query_embed, pos_embed):
        ...
        src = src.permute(0,2,1,3,4).flatten(0,1).contiguous() # bsxt, c, h, w
        src = src.flatten(2).permute(2, 0, 1) # hw, bsxt, c

        pos_embed = pos_embed.permute(0,2,1,3,4).flatten(0,1).contiguous()
        pos_embed = pos_embed.flatten(2).permute(2, 0, 1)

        query_embed = query_embed.unsqueeze(1).repeat(1, bs*t, 1)
        mask = mask.flatten(2).flatten(0,1)
