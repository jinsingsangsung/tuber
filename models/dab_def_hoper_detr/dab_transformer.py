...
class Transformer(nn.Module):
    ...
    def forward(self, src, mask, refpoint_embed, pos_embed):
        ...
        # prepare input for deformable transformer
        spatial_shapes = [(h,w)]
        spatial_shapes = torch.as_tensor(spatial_shapes, dtype=torch.long, device=memory.device)
        level_start_index = torch.cat((spatial_shapes.new_zeros((1, )), spatial_shapes.prod(1).cumsum(0)[:-1]))
        valid_ratios = torch.stack([self.get_valid_ratio(m) for m in masks], 1) # BT, 1, 2
        ...
        if self.eff:
            ...
            valid_ratios = valid_ratios.reshape(bs, t, 1, 2)[:, t//2:t//2+1, :, :].flatten(0,1)
        else:
        ...
        hs, references, cls_hs = self.decoder(loc_tgt, cls_tgt, memory, spatial_shapes, level_start_index, valid_ratios, memory_key_padding_mask=mask,
...
class TransformerDecoder(nn.Module):
    ...
    def forward(self, loc_tgt, cls_tgt, memory,
        ...
        for layer_id, (layer, cls_layer) in enumerate(zip(self.layers, self.cls_layers)):
            ...
            # else:
            cls_reference_points = reference_points.clone().detach().sigmoid()
            cls_reference_points_input = cls_reference_points.transpose(0,1).contiguous()[:, :, None] \
                                         * torch.cat([valid_ratios, valid_ratios], -1)[:, None] # bs, nq, n_level, 4
            ...
            cls_output = cls_layer(cls_output, memory, tgt_mask=tgt_mask,
                                #    memory_mask=memory_mask,
                                   tgt_key_padding_mask=tgt_key_padding_mask,
                                   memory_key_padding_mask=memory_key_padding_mask,
                                #    pos=pos,
                                   cls_query_pos=None,
                                #    cls_query_sine_embed=cls_query_sine_embed,
                                   reference_points = cls_reference_points_input,
...
class TransformerClsDecoderLayer(nn.Module):
    ...
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1,
        ...
        self.ca_v_proj = nn.Linear(d_model, d_model)
        self.ca_qpos_sine_proj = nn.Linear(d_model, d_model)
        self.cross_attn = MSDeformAttn(d_model=d_model, n_levels=1, n_heads=8, n_points=4)
    ...
    def forward(self, tgt, memory,
        ...
        # ========== Begin of Cross-Attention =============
        # Apply projections here
        # shape: num_queries x batch_size*T x 256
        tgt2 = self.cross_attn(self.with_pos_embed(tgt, cls_query_pos).transpose(0,1),
                               reference_points,
                               memory.transpose(0,1), src_spatial_shapes, level_start_index, memory_key_padding_mask)
        ...
        # ========== End of Cross-Attention =============
        tgt = tgt + self.dropout2(tgt2.transpose(0,1).contiguous())
        tgt = self.norm2(tgt)
        tgt2 = self.linear2(self.dropout(self.activation(self.linear1(tgt))))
        tgt = tgt + self.dropout3(tgt2)
        tgt = self.norm3(tgt)
